...
function updateGaugeAndKpis(latest){
  ...
  const flow = pickLatestValue(latest, "flow_lps");
  ...
  // ✅ KPI FLOW (baru)
  $("kpiFlow").textContent = flow === null ? "-" : fmt2(flow);
...
async function refreshCharts(){
  ...
  const KEYS = ["wishner","flow_lps","velocity","battery_soc","batteryVoltage","charging","temperature","humidity","pressure"];
  ...
  try{
    ...
    updateChart(charts.flow, toChartPoints(data.flow_lps));
...
function initChartsOnce(){
  if (!charts.water){
    ...
    charts.flow     = makeLineChart("chartFlow",     "Flow (L/s)");
  ...
  const ids = ["chartWishner","chartFlow","chartVelocity","chartBattSoc","chartBattVolt","chartCharging","chartTemp","chartHum","chartPress"];
  for (const id of ids){
    const cv = $(id);
    if (!cv) continue;
